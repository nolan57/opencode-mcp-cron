 ...
 * @returns 执行结果
 */
export async function executeJob(job, executionId) {
 ...
 */
async function executeAgentTurn(ctx, job, hardTimeoutMs) {
    const repo = getRepository();
 ...
 */
function startResilientHeartbeat(ctx) {
    console.error(`[Executor] ${ctx.executionId}: Starting resilient heartbeat (interval: ${HEARTBEAT_INTERVAL_MS}ms)`);
    const scheduleNextHeartbeat = () => {
        // 检查是否应该停止心跳
        if (ctx.isCompleted) {
            console.error(`[Executor] ${ctx.executionId}: Heartbeat stopped (execution completed)`);
 ...
 * 容错处理：失败时仅记录警告，不抛异常
 */
function performHeartbeat(ctx) {
    const repo = getRepository();
    try {
        const success = repo.updateHeartbeat(ctx.executionId);
        if (success) {
            // 成功：记录调试日志
            console.error(`[Executor] ${ctx.executionId}: Heartbeat updated successfully`);
        }
        else {
            // 返回 false 通常意味着执行已不在 running 状态
            console.warn(`[Executor] ${ctx.executionId}: Heartbeat update returned false (execution may have changed status)`);
        }
    }
    catch (error) {
        // 失败：仅记录警告，绝不抛异常，确保定时器链条不中断
        const errorMsg = error instanceof Error ? error.message : String(error);
        console.warn(`[Executor] ${ctx.executionId}: Heartbeat update failed (will retry): ${errorMsg}`);
 ...
 */
function handleHardTimeout(ctx, resolve, timeoutMs) {
    // 检查是否已完成
    if (ctx.isCompleted) {
        console.error(`[Executor] ${ctx.executionId}: Hard timeout triggered but execution already completed`);
        return;
    }
    console.error(`[Executor] ${ctx.executionId}: HARD TIMEOUT reached (${timeoutMs / 1000}s), forcing SIGKILL`);
    ctx.isCompleted = true;
    ...
    if (ctx.process) {
        try {
            ...
            const errorMsg = error instanceof Error ? error.message : String(error);
            console.error(`[Executor] ${ctx.executionId}: Failed to kill process: ${errorMsg}`);
        }
    }
    // 立即返回结果（不等待 close 事件）
    resolve({
 ...
 * 清理执行上下文
 */
function cleanupExecutionContext(ctx) {
 ...
 */
export function applyJobResult(job, result, nowMs) {
    const updates = {
        state: { ...job.state },
    };
    // 统一处理状态
 ...
 */
export function detectStaleExecutions(timeoutMs = 5 * 60 * 1000) {
    const repo = getRepository();
 ...
 * 取消执行
 */
export function cancelExecution(executionId, reason) {
 ...
 * 暂停执行
 */
export function pauseExecution(executionId, reason) {
    const ctx = activeExecutions.get(executionId);
    if (!ctx) {
        return false;
    }
 ...
 * 获取活跃执行数量
 */
export function getActiveExecutionCount() {
