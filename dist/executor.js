...
async function executeAgentTurn(job, logFile) {
    const startTime = Date.now();
...
async function executeSystemEvent(job, logFile) {
    return {
        status: 'ok',
...
export function applyJobResult(job, result, nowMs) {
    ...
    if (job.schedule.kind === 'at') {
        ...
        const idx = Math.min((updates.state.consecutiveErrors || 1) - 1, ERROR_BACKOFF_MS.length - 1);
