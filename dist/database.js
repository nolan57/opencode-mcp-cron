...
export class DatabaseManager {
   ...
   */
    ...
    getSchemaVersion() {
        ...
        const row = this.db.prepare('SELECT MAX(version) as version FROM schema_versions').get();
    ...
    getStats() {
        ...
        const pageSize = this.db.pragma('page_size', { simple: true }).page_size;
        const pageCount = this.db.pragma('page_count', { simple: true }).page_count;
        return {
            dbPath: this.dbPath,
            journalMode,
            pageSize,
...
export class MigrationError extends Error {
    version;
    cause;
    constructor(message, version, cause) {
        super(message);
    ...
    if (!dbInstance) {
        try {
            ...
            if (error instanceof Error) {
                throw new MigrationError(`Database initialization failed: ${error.message}`, undefined, error);
            }
