...
class CronScheduler {
    ...
    async runLoop() {
        ...
        if (dueResult.hasRemaining) {
            ...
            console.error(`[Scheduler] ${dueResult.totalDue} tasks due, processing batch of ${dueResult.batch.length}. ` +
     ...
     */
    getDueJobsWithLimit(availableSlots) {
        const nowMs = Date.now();
     ...
     * 处理需要审批的任务
     */
    async handleApprovalRequired(job) {
        // 创建执行记录
        ...
        // 更新执行状态为等待审批
        this.repo.transitionExecutionStatus(execution.id, 'pending', 'waiting_for_approval', {});
        // 更新任务下次执行时间
     ...
     * 处理执行错误
     */
    handleExecutionError(job, errorMsg) {
        ...
        // 使用错误退避策略
        const { getErrorBackoffMs } = require('./types.js');
     ...
     * 批准执行
     */
    approveExecution(executionId, note) {
     ...
     * 获取执行历史
     */
    getExecutionHistory(jobId, limit = 10) {
        return this.repo.listExecutionsByJob(jobId, limit);
    }
    /**
     * 获取执行日志
     */
    getExecutionLogs(executionId, limit = 100, offset = 0) {
