        ...
        switch (name) {
            ...
            case 'cron_add': {
                ...
                if (inputArgs.schedule?.kind === 'cron') {
                    const expr = inputArgs.schedule.expr;
                    if (expr) {
            ...
            case 'cron_get_logs': {
                const inputArgs = args;
                const result = scheduler.getExecutionLogs(inputArgs.executionId, inputArgs.limit || 100, inputArgs.offset || 0);
                return {
                    content: [
                        {
                            type: 'text',
